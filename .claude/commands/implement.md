...
## When done
1. Write results to `projects/[folder]/docs/audit-log.md` — append iteration entry
2. Update `projects/[folder]/docs/plan.md` — check off completed items
3. **Git commit**: ensure git is initialized in the project folder (run `git init` + create `.gitignore` if `.git/` doesn't exist), then `git add -A && git commit -m "Iteration N: [summary]"`. Never commit `.env` files.
...
For each active idea, check if `projects/<name>/.git` exists. If NOT:
1. Run `git init` in `projects/<name>/`
2. Create a `.gitignore` with standard Node ignores (`node_modules/`, `dist/`, `.env`, `.next/`, `*.log`)
3. Run `git add -A && git commit -m "Initial commit"` to capture any existing files

**Include this check in each agent brief** — the agent must verify git is initialized before doing any work, and initialize it if missing.
...
### V2/Growth phase (continuous after launch — see Step 7):

Re-run any skill as needed. Common re-runs:
- **`page-cro`** — re-optimize landing after real traffic data
- **`social-content`** — monthly content refresh
- **`frontend-design`** — V2 feature UIs
- **`paywall-upgrade-cro`** — optimize free→paid conversion
...
### C) Git commit all changes in the project folder

**Every agent MUST commit at the end of its iteration.** This is non-negotiable.

1. **Check if git is initialized**: if `projects/<name>/.git` does NOT exist:
   - Run `git init` in `projects/<name>/`
   - Create `.gitignore` with: `node_modules/`, `dist/`, `.env`, `.next/`, `*.log`, `.turbo/`, `coverage/`
   - `git add -A && git commit -m "Initial commit"`
2. **Stage and commit all changes**:
   ```bash
   cd projects/<name>/
   git add -A
   git commit -m "Iteration N: [one-line summary of what was done]"
   ```
   - Commit message examples: `"Iteration 3: build changelog crawler service"`, `"Iteration 6: landing page with pricing and FAQ"`, `"Iteration 8: polish gate 2 — copywriting pass"`
   - If nothing changed (no diff), skip the commit — don't create empty commits
   - **Never commit `.env` files** — only `.env.example`. If `.env` exists, ensure it's in `.gitignore`
...
**When 9/10 or 10/10 — enter POLISH phase (10 audit/polish iterations).**

Update TRACKER.md status to `POLISHING`. Run ONE gate per iteration — they are separate iterations, not one batch. Each gate: run the skill, apply fixes, record results in audit-log as `Polish gate X/10: [skill-name] — PASS/FAIL`.

| Gate | Skill | Focus |
|:----:|-------|-------|
| 1 | **`page-cro`** | Landing page conversion — headline, CTA, social proof, layout |
| 2 | **`copywriting`** | Sharpen ALL copy — landing page, onboarding, emails, error messages |
| 3 | **`seo-audit`** | Meta tags, OG images, structured data, sitemap, performance |
| 4 | **`a11y-audit`** | WCAG Level A/AA — contrast, alt text, keyboard nav, ARIA |
| 5 | **`frontend-design`** | UI polish pass — spacing, typography, loading states, empty states, micro-interactions |
| 6 | **`signup-flow-cro`** | Auth/signup friction — reduce steps, improve error messages, social login |
| 7 | **`form-cro`** | All data-entry forms — field order, validation UX, progressive disclosure |
| 8 | **`onboarding-cro`** | First-run experience — time-to-value, activation checklist, empty states |
| 9 | **`webapp-testing`** | Playwright smoke tests — happy paths, error paths, edge cases |
| 10 | **`simplify`** | Code quality — dead code, duplication, performance, bundle size |

Track gate progress in audit-log: `Polish gates: 7/10 passed`
...
## STEP 7: V2 / GROWTH (continuous — runs indefinitely after launch)

**Trigger:** Status is `LAUNCHED`. Update status to `GROWING`.

This phase **never auto-terminates**. Each iteration, the agent picks the highest-impact task from these categories (in priority order):

### Priority 1: Fix what's broken
- Bug reports (check audit-log for known issues)
- Build failures, broken features, security issues
- Performance regressions

### Priority 2: Conversion optimization
- Analyze funnel: landing page → signup → activation → payment
- Run `page-cro` on underperforming pages (re-run every 5 iterations if conversion data available)
- Improve onboarding based on drop-off points
- A/B test pricing copy, CTA placement

### Priority 3: Feature expansion (V2)
- Read `docs/plan.md` — add a "## V2 Features" section if it doesn't exist
- Prioritize features by: user requests > retention drivers > nice-to-haves
- One feature per iteration, same build→audit→track cycle as Phase 1
- Examples: new integrations, dashboard improvements, team features, mobile responsive fixes, API docs
...
```markdown
## Iteration N — YYYY-MM-DD [V2/GROWTH]
### Priority picked: [1-4] — [reason]
### What was done
- [changes]
### Impact
- [what improved — conversion, UX, performance, reach]
### Skills used: [skill-name or "none"]
### What's next
- [next highest-impact task]
```

### When to output `[LOOP_COMPLETE]`:
**Only when ALL active ideas are either KILLED or STABLE.** An idea reaches `STABLE` status when:
- It has been in GROWING phase for 10+ iterations
- No critical bugs or build failures in the last 5 iterations
- The agent's "What's next" consistently returns low-impact tasks (Priority 4 only)
- The orchestrator decides further iteration has diminishing returns

Even then, `[LOOP_COMPLETE]` is a suggestion — the loop can be restarted anytime with `/loop 30m /implement`.
...
0. **NEVER reference competitors by name in user-facing code** (landing pages, UI, emails, meta tags, copy). Use generic descriptions instead: "your old tool shut down", "free tools with clunky UX", "enterprise platforms that cost $300+/mo". Competitor names belong ONLY in internal docs (`docs/competitive-teardown.md`, `docs/plan.md`, `CLAUDE.md`). This applies to ALL skills that generate copy (`landing-page-generator`, `copywriting`, `page-cro`, `social-content`, etc.) — always instruct the skill to avoid naming competitors.
...
12. **The loop runs indefinitely through BUILD → POLISH → LAUNCH → GROW phases.** Only output `[LOOP_COMPLETE]` when ALL ideas are STABLE or KILLED. MVP_COMPLETE is NOT the end — it's the beginning of polish, launch, and growth.
